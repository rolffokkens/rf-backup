...
install:
	...
	$(DESTDIR)/usr/share/rf-backup $(DESTDIR)/etc
	...
	install -m 755 rf-backup.lib.sh $(DESTDIR)/usr/share/rf-backup.lib.sh
	install -m 644 rf-backup@.service $(DESTDIR)/usr/lib/systemd/system/rf-backup@.service
	install -m 644 rf-backup.conf $(DESTDIR)/etc/rf-backup.conf
